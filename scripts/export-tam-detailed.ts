...
async function main() {
	// Get TAM form
	const tamFormId = (await db.execute("SELECT id FROM forms WHERE title LIKE '%TAM%'")).rows[0]!.id as string;
	...
	// SD
	const puSd = Math.sqrt(puAverages.map(x => (x - puMean) ** 2).reduce((a, b) => a + b, 0) / (puAverages.length - 1));
	const peuSd = Math.sqrt(peuAverages.map(x => (x - peuMean) ** 2).reduce((a, b) => a + b, 0) / (peuAverages.length - 1));
	console.log(`PU SD = ${puSd.toFixed(2)}`);
	console.log(`PEoU SD = ${peuSd.toFixed(2)}`);

	// Get feedback responses
	const fbFormId = (await db.execute("SELECT id FROM forms WHERE title LIKE '%Feedback%'")).rows[0]!.id as string;
