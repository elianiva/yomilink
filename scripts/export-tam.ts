...
async function main() {
	// Get TAM form
	const tamForm = (await db.execute("SELECT id FROM forms WHERE type = 'questionnaire' LIMIT 1"))
		.rows[0];
	...
	const feedbackForm = (
		await db.execute({
			sql: "SELECT id FROM forms WHERE type = ? ORDER BY created_at DESC LIMIT 1",
			args: ["questionnaire"],
		})
	).rows[1];
	if (feedbackForm) {
