...
async function main() {
	...
	for (const s of students.rows) {
		...
		if (ageCount <= 5) {
			console.log(
				`  ${s.email}: age=${age}, JLPT=${jlpt}, months=${duration}, prevScore=${score}, media=${media}h/w, group=${group}`,
			);
		}
	}
	...
	if (durCount > 0)
		console.log(
			`Study duration (months): mean=${(durSum / durCount).toFixed(1)}, n=${durCount}`,
		);
	if (scoreCount > 0)
		console.log(
			`Previous Japanese score: mean=${(scoreSum / scoreCount).toFixed(1)}, n=${scoreCount}`,
		);
	if (mediaCount > 0)
		console.log(
			`Media consumption (hrs/wk): mean=${(mediaSum / mediaCount).toFixed(1)}, n=${mediaCount}`,
		);
