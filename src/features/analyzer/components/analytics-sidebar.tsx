...
export function AnalyticsSidebar({
	...
	const groupedLearners = useMemo(() => {
		const isSummaryLearner = (learner: LearnerAnalytics) => learner.condition === "summarizing";
		return {
	...
	const handleToggleAll = (checked: boolean | "indeterminate") => {
		onToggleAll(
			checked === true,
			activeTabLearners.map((l: LearnerAnalytics) => l.learnerMapId),
		);
	};
						...
						<>
							...
							>
								<TabsList variant="line" className="w-full">
									<TabsTrigger value="conceptMap" className="text-xs px-2 flex-1">
										Concept Map ({counts.conceptMap})
									</TabsTrigger>
									<TabsTrigger value="summary" className="text-xs px-2 flex-1">
