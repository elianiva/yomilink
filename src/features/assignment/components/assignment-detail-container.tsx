...
export function AssignmentDetailContainer() {
	...
	const { data: assignment, rpcError, isLoading } = useRpcQuery(
		AssignmentRpc.getAssignmentById(assignmentId)
	);

	if (isLoading) return <AssignmentDetailSkeleton />;
	if (rpcError) return <ErrorCard title="Failed to load assignment" description={rpcError} />;
	if (!assignment) return <NotFoundCard resource="Assignment" />;
